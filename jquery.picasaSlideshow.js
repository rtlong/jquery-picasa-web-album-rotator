 ...
 *  <script src="jquery.picasaSlideshow.js" type="text/javascript"></script>
 *  <script type="text/javascript">
 *    $(document).ready(function() {
 *        $(".photoSlider").each(function(i){
 *            $(this).picasa_slideshow(
 *                $.extend({
 *                    username: $(this).data('username') || 'default@domain.com', // the email address of the album owner
 *                    album: $(this).data('album'), // the name of the album on PicasaWeb
 *                    imgMax: $(this).data('imgMax') || 800, // the largest an image should be in either dimension
 *                    whenDone: function(populated_element) {
 *                        populated_element.cycle({
 *                            // see http://jquery.malsup.com/cycle/ for more options on the Cycle plugin
 *                            fx: 'fade'
 *                        });
 *                    }
 *                },
 ...
 *     }
 *  </style>
 *  <div class="photoSlider" style='width: 568px; height: 568px'
 *    data-album='My Album' data-imgMax='800' />
 */
 ...
 * @bastardized_by Ryan Long <ryan@rtlong.com> - needed a light version to pull in img's for the cycle plugin
 * @revision 2.0.0 Beta
...
(function ($) {
    ...
    $.fn.picasa_slideshow = function (opts) {
        var $self, settings = {};
        opts = $.extend(true,{}, $.fn.picasa_slideshow.defaults, opts);
        ...
        function photo(j) {
            var $html = $("<img />").attr({
                src:    j.media$group.media$content[0].url,
                height: j.media$group.media$content[0].height,
                width:  j.media$group.media$content[0].width
            });
        ...
        function getAlbum() {
            if (settings.photostore[settings.album]) {
                ...
                var $u = [
                    [ 'http://picasaweb.google.com/data/feed/api/user', settings.username,
                      'album', settings.album ].join('/'),
                    ['kind=photo', 'alt=json', 'imgmax='+settings.imgMax].join('&')
                ].join('?');
                console.log($u);
